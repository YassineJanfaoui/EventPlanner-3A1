...
public class AjouterEvent {
    ...
    void Validate(ActionEvent event) throws IOException {
        ...
        if (name.getText().isEmpty() ||
                ...
                fee.getText().isEmpty() ||
                userId.getValue() == null ) {  // Check if the image path is not empty
        ...
        // Check if start date is in the future (after today)
        if (!start.isAfter(today)) {
            Alert alert = new Alert(Alert.AlertType.ERROR);
            alert.setTitle("Erreur");
            alert.setContentText("La date de début doit être une date future!");
            alert.show();
            return;
        }
    ...
    @FXML
    private void goToAjouterEven(ActionEvent event) {
        try {
            // Charger la nouvelle page (ajoutez le bon chemin FXML)
